...
use crate::{
    ...
    Transaction, TransactionDirection, ConfirmedTransaction,
...
impl BitcoinTransaction {
    ...
    pub fn try_from_str(tx_hex: &str) -> Result<Self> {
        ...
        Err(BitcoinTransactionError::ParseTransactionError("Unable to parse transaction".into()))
    }
}
...
impl BitcoinTransaction {
    ...
    pub fn _try_from_nfc_message(nfc_message: std::sync::Arc<NfcMessage>) -> Result<Self> {
...
impl ConfirmedTransaction {
    ...
    #[uniffi::method]
    pub fn fiat_amount(&self) -> Option<cove_fiat::FiatAmount> {
        self.fiat
    }
}
...
fn transaction_preview_confirmed_new() -> Transaction {
    ...
    let txn = ConfirmedTransaction {
        ...
        fiat: Some(cove_fiat::FiatAmount::preview_new()),
        labels: Default::default(),
    };

    Transaction::Confirmed(std::sync::Arc::new(txn))
}
...
fn transaction_preview_unconfirmed_new() -> Transaction {
    ...
    let rand_hours = rng.gen_range(0..4);
    let rand_minutes = rng.gen_range(0..60);
    
    // Convert hours and minutes to seconds
    let seconds_ago = (rand_hours * 3600) + (rand_minutes * 60);
    
    let last_seen = jiff::Timestamp::now()
        .checked_sub(jiff::FixedOffset::seconds(seconds_ago as i32))
        .unwrap()
        .as_second()
        .try_into()
        .unwrap();

    Transaction::Unconfirmed(std::sync::Arc::new(UnconfirmedTransaction {
        txid: TxId::preview_new(),
        sent_and_received: SentAndReceived::preview_new(),
        last_seen,
        fiat: None,
