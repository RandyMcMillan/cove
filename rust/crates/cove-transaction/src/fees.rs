...
impl FeeRateOptionsWithTotalFee {
    ...
    pub fn transaction_size(&self) -> u64 {
        ...
        let fast_size = fast_total_fee_in_kwu / self.fast.fee_rate.0.to_sat_per_kwu();

        let medium_total_fee_in_kwu = self.medium.total_fee.as_sats() * 250;
        let medium_size = medium_total_fee_in_kwu / self.medium.fee_rate.0.to_sat_per_kwu();

        let slow_total_fee_in_kwu = self.slow.total_fee.as_sats() * 250;
        let slow_size = slow_total_fee_in_kwu / self.slow.fee_rate.0.to_sat_per_kwu();
    ...
    pub fn calculate_custom_fee_speed(&self, fee_rate: f32) -> FeeSpeed {
        ...
        let fast_fee_rate = self.fast.fee_rate.0.to_sat_per_kwu();
        let medium_fee_rate = self.medium.fee_rate.0.to_sat_per_kwu();
        let slow_fee_rate = self.slow.fee_rate.0.to_sat_per_kwu();
...
// Color type for fee speed visualization
#[derive(Debug, Clone, Copy, PartialEq, Eq, uniffi::Enum)]
pub enum FfiColor {
    Green(FfiColorComponent),
    Yellow(FfiColorComponent),
    Orange(FfiColorComponent),
    Red(FfiColorComponent),
    Blue(FfiColorComponent),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, uniffi::Record)]
pub struct FfiColorComponent {
    pub r: u8,
    pub g: u8,
    pub b: u8,
}