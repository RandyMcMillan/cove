...
use super::{ELECTRUM_BATCH_SIZE, Error, NodeClientOptions, run_blocking};
use crate::node::Node;
...
impl ElectrumClient {
    ...
    pub async fn get_transaction(
        ...
        let client = self.client.clone();
        let transaction = run_blocking(move || {
            client
                .inner
    ...
    pub async fn sync(
        ...
        let result = run_blocking(move || client.sync(request, batch_size, false))
            .await
            .map_err(Error::ElectrumScan)?;
