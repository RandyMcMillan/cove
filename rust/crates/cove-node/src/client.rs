...
use super::client_builder::NodeClientBuilder;
...
pub enum Error {
    ...
    #[error("failed to complete wallet scan: {0}")]
    EsploraScan(esplora_client::Error),
    ...
    // Added for unblock-related functions
    #[error("blocking operation failed: {0}")]
    BlockingOperationFailed(String),
}
...
// Simplified unblock function to break dependency on main crate
    ...
    pub async fn new(node: &Node) -> Result<Self, Error> {
        match node.api_type {
            ...
            ApiType::Rpc => {
                ...
                Err(Error::BlockingOperationFailed("RPC support not implemented".to_string()))
            }
        }
    }
